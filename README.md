...
- Button 1: Log Progress
  ...
  - Pop-up 1: 'habit_name' was successfully logged current streak 'habit_streak'.
    ...
    This pop-up is displayed if a daily habit is logged within streak+1 days since the start of the streak. This affords a user the ability to miss the habit once without the streak resetting.

    This pop-up is displayed if a weekly habit is logged within streak*7 days 12-hour since the start of the streak. Weekly habits have a 12-hour grace period.
  - Pop-up 2: The current streak for habit_name has reset was 'habit_streak' currently 1.
    
    This pop-up is displayed if a daily habit is not logged within streak+1 days since the start of the streak. The previous streak value is set to 1.

    This pop-up is displayed if a weekly habit is not logged within streak*7 days 12-hour since the start of the streak. The previous streak value is set to 1.
...
# Test suite
The test_suite folder is for verification purposes only, users are free to delete the folder.
